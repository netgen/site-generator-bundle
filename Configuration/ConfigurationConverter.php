...
class ConfigurationConverter
{
    ...
    public function fromLegacy( $projectName, $adminSiteAccess )
    {
        ...
        // Only admin siteaccess has legacy mode
    ...
    public function getGroup( $groupName, $file = null, $siteaccess = null )
    {
        ...
        return $this->legacyKernel->runCallback(
            ...
            {
                // @todo: do reset injected settings every time
        ...
        foreach ( explode( ';', $siteaccessSettings['MatchOrder'] ) as $matchMethod )
        {
            switch ( $matchMethod )
            {
                ...
                case 'host':
                    ...
                    break;
                case 'host_uri':
                    $match = $this->resolveHostURIMatching( $siteaccessSettings );
                    break;
                case 'port':
    ...
    /**
     * Parses Legacy HostUriMatching settings to a matching array
     * @param mixed[] $siteaccessSettings
     *
     * @throws \eZ\Publish\Core\Base\Exceptions\InvalidArgumentException
     *
     * @return array|bool
     */
    protected function resolveHostURIMatching( $siteaccessSettings )
    {
        $matchArray = array(
        ...
        foreach ( $siteaccessSettings['HostUriMatchMapItems'] as $hostUriMatchMapItem )
        {
            $hostUriMatchMapItemArray = explode( ';', $hostUriMatchMapItem );
            if ( count( $hostUriMatchMapItemArray ) !== 3 || empty( $hostUriMatchMapItemArray[0] ) || empty( $hostUriMatchMapItemArray[2] ) )
            {
            ...
            if ( !empty( $hostUriMatchMapItemArray[1] ) )
            {
